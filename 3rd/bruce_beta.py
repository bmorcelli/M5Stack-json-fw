...
# 4. Remover entradas com "author": JSON_AUTHOR dos arquivos JSON em /r
def clean_json_files():
    for filename in os.listdir(JSON_DIR_OUT):
        ...
        # Remove elementos com author == JSON_AUTHOR
        if isinstance(data, list):
            new_data = [item for item in data if item.get("author") != JSON_AUTHOR]
        else:
...
def update_published_at(published_date):
    for filename in os.listdir(JSON_DIR):
        ...
        if isinstance(data, list):
            modified = False
            for item in data:
                if "versions" in item and item.get("author") == JSON_AUTHOR:
